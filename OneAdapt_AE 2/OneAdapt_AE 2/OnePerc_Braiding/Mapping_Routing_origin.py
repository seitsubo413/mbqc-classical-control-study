...
def map_route(graph, dgraph, NQubit, N, Refresh, RefreshBound):
    ...
    while len(graph.nodes()):
        ...
        reverse_map = {}
        swap_nodes = []
        for key in allocated_incomplete_nodes.keys():
            reverse_map[allocated_incomplete_nodes[key]] = key
     
        degree_map = {}
        for key in reverse_map.keys():
            degree_map[key] = []
            neigh_knodes = graph.neighbors(key)
        ...
        for key in keys:
            if key not in swap_nodes:
                ...
                if dpos != 0:
                    if reverse_map[key] + dpos in allocated_incomplete_nodes.keys():
                        if allocated_incomplete_nodes[reverse_map[key] + dpos] not in swap_nodes:
                            swap_nodes.append(key)
                            swap_nodes.append(allocated_incomplete_nodes[reverse_map[key] + dpos])
                            reverse_map[allocated_incomplete_nodes[reverse_map[key] + dpos]] = reverse_map[key]
                            allocated_incomplete_nodes[reverse_map[key]] = allocated_incomplete_nodes[reverse_map[key] + dpos]
                            allocated_incomplete_nodes[reverse_map[key] + dpos] = key
                            reverse_map[key] += dpos                   
                    else:
                        swap_nodes.append(key)
                        del allocated_incomplete_nodes[reverse_map[key]]
                        allocated_incomplete_nodes[reverse_map[key] + dpos] = key
                        reverse_map[key] += dpos
        ...
        if len(swap_nodes) == 0:
            degree_map = {}
            for key in reverse_map.keys():
                degree_map[key] = len(list(graph.neighbors(key)))
            ...
            for key in keys:
                if key not in swap_nodes:
                    ...
                    for dkey in distance_map.keys():
                        ...
                        y = cur_pos % N
                        if x > 0:
                            if cur_pos - N not in allocated_incomplete_nodes.keys():
                                distance_map[dkey] += 1
                        if x < N - 1:
                            if cur_pos + N not in allocated_incomplete_nodes.keys():
                                distance_map[dkey] += 1
                        if y > 0:
                            if cur_pos - 1 not in allocated_incomplete_nodes.keys():
                                distance_map[dkey] += 1
                        if y < N - 1:
                            if cur_pos + 1 not in allocated_incomplete_nodes.keys():
                                distance_map[dkey] += 1
