...
def generate_graph_state(gates_list, qubits):
    ...
    for gate in gates_list:
        ...
        # gate type is CZ
            ...
            if pre_nodes[qubit1] == -1:
                ...
                if pre_gate[qubit1] == 1:
                    pre_node = pre_nodes[qubit1]
                    graph.add_node(node_index, node_val = "Out", pos = (pos_x, - qubit1), phase = 0, qubit = qubit1)
                    graph.nodes[pre_node]['phase'] = -1
                    graph.add_edge(pre_node, node_index)
                    pre_nodes[qubit1] = node_index
                    pos_x += 3
                    node_index += 1
            ...
            if pre_nodes[qubit2] == -1:
                ...
                if pre_gate[qubit2] == 1:
                    pre_node = pre_nodes[qubit2]
                    graph.add_node(node_index, node_val = "Out", pos = (pos_x, - qubit2), phase = 0, qubit = qubit2)
                    graph.nodes[pre_node]['phase'] = -1
                    graph.add_edge(pre_node, node_index)
                    pre_nodes[qubit2] = node_index
                    pos_x += 3
    ...
    #     if depths[q] < max_depth:
    #         for i in range(depths[q], max_depth):
    #             pre_node = pre_nodes[q]
    #             graph.add_node(node_index, node_val = "Out", pos = (pos_x, - q), phase = -1, qubit = q)
    #             graph.nodes[pre_node]['phase'] = 1
    #             graph.add_edge(pre_node, node_index)
    #             pre_nodes[q] = node_index
    #             pos_x += 3
    #             node_index += 1
    return graph
