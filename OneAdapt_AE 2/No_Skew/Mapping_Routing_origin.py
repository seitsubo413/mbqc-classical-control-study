...
def one_layer_map_route(origin_graph, dgraph, allocated_nodes_cache, graph_nodes_pos, layer_index, N, required_life_time, Refresh, RefreshBound, NQubit, P, D_Flag, Braiding, nodes_to_be_measured, layer_graph_map, MixedBound, nodes_to_appear, skew_nodes, pre_swap_nodes):
    ...
    if len(skew_nodes):
        for sn in skew_nodes:
            if sn in origin_graph.nodes():
                index += 1
                pre_layer_incomplete_nodes[skew_nodes[sn][1]] = sn
                # pre_layer_incomplete_nodes[skew_nodes[sn][0]] = Auxiliary
                if sn in allocated_nodes_cache:
                    del allocated_nodes_cache[sn]
    # get refresh nodes
    refresh_nodes = []
    # index = 0
    if Refresh:     
            ...
            if layer_index - graph_nodes_pos[gnode] >= RefreshBound and gnode not in refresh_nodes and gnode in allocated_nodes_cache.keys():
                if allocated_nodes_cache[gnode] not in pre_layer_incomplete_nodes.keys():
                    ...
                    if graph_nodes_pos[pre_layer_incomplete_nodes[allocated_nodes_cache[gnode]]] > graph_nodes_pos[gnode]:
                        if pre_layer_incomplete_nodes[allocated_nodes_cache[gnode]] in refresh_nodes:
                ...
                for kkey in akeys:
                    if origin_graph.has_edge(key, kkey) and allocated_nodes_cache[kkey] not in pre_layer_incomplete_nodes.keys():
                        if allocated_nodes_cache[kkey] != allocated_nodes_cache[key]:
                            ...
                            index += 1 
                            flag = True
                        # elif Braiding and not len(skew_nodes):
                        #     direct = []
                        ...
                        #             swap_odd_nodes[kkey] = (allocated_nodes_cache[kkey], allocated_nodes_cache[kkey] + d)
    ...
    for key in pre_layer_incomplete_nodes.keys():
        if pre_layer_incomplete_nodes[key] != Auxiliary:
            reverse_map[pre_layer_incomplete_nodes[key]] = key

    if len(skew_nodes):
        swap_even_nodes = skew_nodes
        for sn in skew_nodes:
            swap_nodes.append(sn)
    else:
        swap_even_nodes = {}
    
    # print(swap_even_nodes)
    
    
    if Braiding and not len(skew_nodes):      
        next_pre_swap_nodes = []
        degree_map = {}
            ...
            if key not in swap_nodes:
                ...
                for dkey in distance_map.keys():
                    for nknode in sorted_degree_map[key]:
                        if nknode in reverse_map:
                            cur_pos = reverse_map[key] + dkey
                            # print("dkey", dkey, "nknode", nknode)
                ...
                if dpos != 0:
                    if reverse_map[key] + dpos in pre_layer_incomplete_nodes.keys() and pre_layer_incomplete_nodes[reverse_map[key] + dpos] != Auxiliary:
                        if pre_layer_incomplete_nodes[reverse_map[key] + dpos] not in swap_nodes:
                            print((key, pre_layer_incomplete_nodes[reverse_map[key] + dpos]))
                            if (key, pre_layer_incomplete_nodes[reverse_map[key] + dpos]) not in pre_swap_nodes and (pre_layer_incomplete_nodes[reverse_map[key] + dpos], key) not in pre_swap_nodes:
                                swap_nodes.append(key)
                                swap_even_nodes[key] = (reverse_map[key], reverse_map[key] + dpos)
                
                                swap_nodes.append(pre_layer_incomplete_nodes[reverse_map[key] + dpos])
                                swap_odd_nodes[key] = (reverse_map[key], reverse_map[key] + dpos)
                                swap_odd_nodes[pre_layer_incomplete_nodes[reverse_map[key] + dpos]] = (reverse_map[key] + dpos, reverse_map[key])

                                swap_pairs.append((reverse_map[key], reverse_map[key] + dpos))
                                swap_pairs.append((reverse_map[key] + dpos, reverse_map[key]))
                                pre_swap_nodes.append((key, pre_layer_incomplete_nodes[reverse_map[key] + dpos]))
                                next_pre_swap_nodes.append((key, pre_layer_incomplete_nodes[reverse_map[key] + dpos]))
                                # del reverse_map[pre_layer_incomplete_nodes[reverse_map[key] + dpos]]
                                # allocated_nodes_cache[pre_layer_incomplete_nodes[reverse_map[key] + dpos]] = reverse_map[key] + dpos
                                # pre_layer_incomplete_nodes[reverse_map[key]] = Auxiliary
            ...
            for key in keys:
                if key not in swap_nodes:
                    ...
                    if dpos != 0:
                        ...
                        #     pre_layer_incomplete_nodes[reverse_map[key] + dpos] = key
                        #     reverse_map[key] += dpos     
        if len(next_pre_swap_nodes) == 0:
            n = len(pre_swap_nodes)
            pre_swap_nodes = random.sample(pre_swap_nodes, n // 3)
    for rnode in pre_layer_incomplete_nodes.values():
        if rnode not in swap_nodes and rnode != Auxiliary:
            swap_pairs.append((reverse_map[rnode], reverse_map[rnode]))

    # for sn in swap_even_nodes:
    #     if not len(skew_nodes):
    #         del reverse_map[pre_layer_incomplete_nodes[swap_even_nodes[sn][0]]]
    #         allocated_nodes_cache[pre_layer_incomplete_nodes[swap_even_nodes[sn][0]]] = swap_even_nodes[sn][1]
    #         pre_layer_incomplete_nodes[swap_even_nodes[sn][0]] = Auxiliary
    # if not len(skew_nodes):
    #     pre_swap_nodes = []
    # if  len(swap_nodes):
    #     pre_swap_nodes = swap_nodes
    ...
    if True:
        for r in range(Round):
            ...
            for nnode in net.nodes():
                if net.nodes[nnode]['node_val'] not in swap_even_nodes and net.nodes[nnode]['node_val'] not in refresh_nodes and net.nodes[nnode]['node_val'] not in extend_nodes:
    ...
    for nnode in nkeys:
        if nnode not in dgraph.nodes():
            ...
            #     return net, {}, layer_graph_map[nodes_to_appear[nnode]][0], layer_graph_map[nodes_to_appear[nnode]][4], [], required_life_time, layer_graph_map[nodes_to_appear[nnode]][3], swap_pairs, cur_force_refresh_nodes_num, layer_graph_map[nodes_to_appear[nnode]][2], layer_graph_map, nodes_to_appear[nnode], layer_graph_map[nodes_to_appear[nnode]][1], layer_graph_map[nodes_to_appear[nnode]][6]          
            del nodes_to_be_measured[nnode]   
    return net, allocated_nodes_incomplete, graph, graph_nodes_pos, inter_edges, required_life_time, allocated_nodes_cache, swap_pairs, cur_force_refresh_nodes_num, nodes_to_be_measured, layer_graph_map, layer_index + 1, dgraph, nodes_to_appear, swap_odd_nodes, pre_swap_nodes
...
def map_route(a, graph, dgraph, NQubit, N, Refresh, RefreshBound, D_Flag, Braiding, P_const):
    ...
    while len(graph.nodes()):
        ...
        net, allocated_incomplete_nodes, graph, graph_nodes_pos, inter_edges, required_life_time,  allocated_nodes_cache, swap_pairs, cur_force_refresh_nodes_num, nodes_to_be_measured, layer_graph_map, layer_index, dgraph, nodes_to_appear, skew_nodes, pre_swap_nodes = one_layer_map_route(graph, dgraph, allocated_nodes_cache, graph_nodes_pos, layer_index, N, required_life_time, Refresh, RefreshBound, NQubit, P, D_Flag, Braiding, nodes_to_be_measured, layer_graph_map, MixedBound, nodes_to_appear, skew_nodes, pre_swap_nodes)
        print(a, old_layer_index, len(list(graph.nodes())))
        print(pre_swap_nodes)
        ...
        if old_layer_index == layer_index - 1:
            ...
            # if succ_rate > 2 and MixedBound < RefreshBound // 2 and MixedBound < 30:
            #     MixedBound += 1
            # MixedBound = RefreshBound // 2
        else:
            succ_rate = 0
        ...
        if NQubit == 64 and layer_index > 10000:
            return net_list, Empty, layer_list, left_graph_nodes_list, inter_edges_list, refresh_begin_list, refresh_end_list, required_life_time, swap_map
        if layer_index > 6000 and same_count > 25:
            return net_list, Empty, layer_list, left_graph_nodes_list, inter_edges_list, refresh_begin_list, refresh_end_list, required_life_time, swap_map
