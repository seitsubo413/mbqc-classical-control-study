...
[Figure 1 — to be generated: MBQC 3-stage pipeline block diagram. The diagram shows three horizontally arranged stages connected by arrows: (1) Issue Stage on the left, labeled with width parameter W and a "ready queue" input; (2) Measurement Stage in the center, labeled with latency L_meas and width meas_width; (3) Feedforward (FF) Stage on the right, labeled with latency L_ff and width F. A solid arrow passes from Issue to Measurement to FF, representing forward data flow. A dashed feedback arrow runs from the FF Stage back to the Issue Stage, labeled "dependency resolved — node unblocked." For ff_rate_matched, a credit token arrow is shown flowing backward from FF to Issue, labeled "credit return on completion." A stall symbol (lock icon) is shown at the Issue Stage gate labeled "stall if ff_in_flight ≥ F." The figure visually contrasts ff_rate_matched (with credit gate) against ASAP (no gate, direct push into FF queue).]
...
[Figure 3 — to be generated: Diagram of the ff_rate_matched credit-based mechanism. The figure contains two horizontal panels. Top panel (ff_rate_matched): Left box labeled "Issue Stage" contains a ready queue and an in-flight counter labeled "ff_in_flight." Right box labeled "FF Stage" shows F labeled slots (depicted as a pipeline of F cells). A forward arrow from Issue to FF is labeled "issue node (consume 1 credit)." A backward arrow from FF to Issue is labeled "credit return (on FF completion)." A stall gate symbol between Issue and FF is shown with a condition label "stall if ff_in_flight ≥ F." Bottom panel (ASAP): Same layout but the stall gate is absent, and a red "OVERFLOW" marker appears at the FF Stage when burst load exceeds F. The visual contrast illustrates that ff_rate_matched prevents overflow by construction while ASAP reacts after the fact.]
...
**Key structural property.** In MBQC circuits with $D_{\mathrm{ff}}^{\mathrm{shifted}} = 1$–$2$, not every issued node has a feedforward dependent. Feedforward edges in the shifted DAG connect only nodes whose measurement outcomes actively determine the basis of a subsequent measurement. A structural property of the MBQC circuit family we study (QAOA, VQE) is that FF nodes constitute at most approximately half of all issued nodes: the remaining nodes are either measurement outcomes that trigger no correction, or nodes whose corrections have been fully absorbed by signal shift. This structural half-fraction is the essential input to the argument below.

**Flow rate argument.** Under ff\_rate\_matched, the credit gate enforces `ff_in_flight ≤ F` at all times. When $D_{\mathrm{ff}}^{\mathrm{shifted}} = 1$–$2$, at most $F$ nodes can be simultaneously in-flight in the FF pipeline. The issue stage issues at most $W$ nodes per cycle. For pipeline stability, the long-run FF arrival rate must not exceed the FF departure rate. Since FF nodes constitute at most half of all issued nodes (the structural property above), the effective FF arrival rate is at most $W/2$ nodes per cycle. Setting $F = \lceil W/2 \rceil$ ensures the FF pipeline operates at capacity utilization $\leq 1$, preventing queue overflow.

**Little's Law interpretation.** This is a direct application of Little's Law [Little1961]: at steady state, mean queue length = arrival rate $\times$ mean latency. For the FF subsystem, $\mathbb{E}[{\tt ff\_in\_flight}] = \lambda_{\mathrm{FF}} \cdot L_{\mathrm{ff}}$. With $\lambda_{\mathrm{FF}} \leq W/2$ and $F = W/2$, Little's Law gives $\mathbb{E}[{\tt ff\_in\_flight}] = (W/2) \cdot L_{\mathrm{ff}} / F = L_{\mathrm{ff}}$, which equals the pipeline depth. This means the FF queue operates at nominal depth — no transient accumulation, no overflow.

**Formal statement.**

> **Theorem 1.** Under ff\_rate\_matched with $F = \lceil W/2 \rceil$, the FF queue length is bounded above by $F$ for any circuit with $D_{\mathrm{ff}}^{\mathrm{shifted}} \geq 1$, regardless of $L_{\mathrm{ff}}$, $L_{\mathrm{meas}}$, or circuit scale ($H$, $Q$). Consequently, the stall regression with respect to the raw DAG baseline is zero across all tested configurations.

The bound $\mathbb{E}[{\tt ff\_in\_flight}] \leq F$ holds by the credit gate construction. The zero-regression claim follows from the flow rate argument: with $F = W/2$, the FF pipeline can accept $W/2$ new arrivals per cycle and complete $W/2$ operations per cycle, matching the structural arrival rate of at most $W/2$ FF nodes per cycle. No backlog accumulates, so no issue-stage stall occurs. Residual stall rates of 0.05–0.15\% observed at finite $N$ in the experiments (Table V-E) are consistent with finite-$N$ boundary effects at the beginning and end of computation, where the steady-state flow assumption is temporarily violated; these residuals vanish as $N$ grows.

*Note on the formula $F \geq W \cdot L_{\mathrm{ff}} / (L_{\mathrm{ff}}+1)$ from draft v1.* The reviewer correctly identified that this formula would imply $F > W/2$ for $L_{\mathrm{ff}} \geq 2$, contradicting the experimental results. That formula was incorrect and has been removed. The correct argument above does not depend on $L_{\mathrm{ff}}$ at all: the credit gate bounds `ff_in_flight ≤ F` regardless of $L_{\mathrm{ff}}$, and the structural half-fraction property gives $\lambda_{\mathrm{FF}} \leq W/2$ regardless of $L_{\mathrm{ff}}$. Therefore $F = W/2$ suffices for all $L_{\mathrm{ff}}$.

**Intuition for the $F/W = 0.125$ case.** At extreme credit tightness ($F=2$, $W=16$, $F/W=0.125$), the credit gate fires whenever more than 2 nodes are in-flight. Despite this aggressive throttling, Study 17 finds cycles\_ratio = 1.000 in all cases. This is because $D_{\mathrm{ff}}^{\mathrm{shifted}} = 1$–$2$ means the condition `ff_in_flight ≥ F` is almost never satisfied in practice: the FF pipeline drains so rapidly (in 1–2 cycles) that `ff_in_flight` rarely reaches $F=2$. The DAG structure, rather than the FF capacity, limits throughput. The credit gate imposes no penalty because the steady-state FF arrival rate is well below $F$.

The empirical evidence strongly supports Theorem 1. In all 1,080 paired comparisons (Study 20), stall rate for ff\_rate\_matched at $F = 4 = W/2$ (with $W = 8$) is below 0.5\%, while ASAP at the same $F$ exhibits 39–49\% stall.
...
Little's Law [Little1961] provides the theoretical foundation for the $F^* = W/2$ result. Applied to the FF pipeline subsystem: at steady state, mean occupancy $L = \lambda \cdot W_{\mathrm{service}}$, where $\lambda$ is the throughput and $W_{\mathrm{service}}$ is the mean service time. With $L \leq F$ enforced by credit-gating and $W_{\mathrm{service}} = L_{\mathrm{ff}}$, the bound $\lambda \leq F/L_{\mathrm{ff}}$ follows directly. The $F = W/2$ threshold corresponds to the structural half-fraction of FF arrivals.
...
**Probabilistic measurement outcomes.** The current model assumes all measurements generate FF work. If the fraction of correction-triggering measurements is $p < 1$, the effective arrival rate is $pW/2$, and $F^*$ could be as low as $\lceil pW/2 \rceil$. Characterizing $p$ empirically for QAOA and VQE circuits would sharpen the hardware sizing guideline.
...
The key theoretical contribution is the characterization of the minimum FF width threshold: $F^*({\tt ff\_rate\_matched}) = \lceil W/2 \rceil$, derived from a Little's Law flow balance argument and confirmed across four experimental axes spanning 4,120 simulation runs:
